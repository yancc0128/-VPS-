...
- 勾选了**「执行前预览并确认所有命令」**（默认开启）时，会先弹出整套部署各步骤将在
  VPS 上执行的**完整脚本预览**，确认后才连接服务器；取消则不会建立任何连接。
- 确认后建立**一条 SSH 会话贯穿全部 9 步**（单连接复用），每步输出**实时流式**显示在
  控制台，可随时点「取消当前步骤」中断。

每一步远程统一用 `sudo -S -p '' bash -lc <脚本>` 执行（`system-check` 例外，不提权）。
脚本里的协议配置以 **base64 传输、远程解码写入**，防注入。
